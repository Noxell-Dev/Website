/**
 * Aurora — vanilla port of the React Bits <Aurora /> (ogl).
 * Fixed full-viewport background in brand colors: ultramarine #1800ad,
 * signal red #ff0000 (mid stop is their 50/50 blend #8b0056).
 * Static single frame under prefers-reduced-motion; dark solid if no WebGL.
 */
...
export function initAurora(): void {
  ...
  try {
    ...
    return; // no WebGL: the solid dark background is the fallback
  }
  ...
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    program.uniforms.uTime.value = 2.5; // one static, on-brand frame
